...
'''
# mauvais départ dates aux plus tot 
Dateauplustot: list[Any] = []
for nod in G.nodes() :
    for pred in G.predecessors(nod):
        for weight in int(pred[0][2]):
            if max_value is None or Dateauplustot[pred] > max_value:
                max_value = num
            Dateauplustot[nod]= max_value+Dateauplustot[pred]
